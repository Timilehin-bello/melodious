...
export const useShouldShowAds = () => {
  ...
  const { hasActiveSubscription, isPremiumUser, isLoading: subscriptionLoading } = useSubscriptionStatus();
...
export const useAdDisplayLogic = () => {
  ...
  const { hasActiveSubscription, isPremiumUser } = useSubscriptionStatus();
  
  const canShowAds = shouldShowAds?.shouldShow && 
