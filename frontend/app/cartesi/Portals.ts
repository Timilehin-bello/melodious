...
export const transferNftToPortal = async (
  ...
  try {
    if (rollups && provider) {
      ...
      const erc721PortalAddress =
        await rollups.erc721PortalContract.getAddress();
  ...
  try {
    if (rollups && provider) {
      ...
      const erc1155SinglePortalAddress =
        await rollups.erc1155SinglePortalContract.getAddress();
  ...
  try {
    if (rollups && provider) {
      ...
      const erc1155BatchPortalAddress =
        await rollups.erc1155BatchPortalContract.getAddress();
      ...
      if (!currentApproval) {
        ...
        if (!event) {
          throw Error(
            `could set approval for DAppERC1155Portal(${erc1155BatchPortalAddress})  (signer: ${signerAddress}, tx: ${receipt?.hash})`
          );
        }
      }
